...
class ArrayLinkedList:
    ...
    def __init__(self, capacity: int):
        ...
        self.n = [Node()] * capacity    # 리스트 본체  self.n의 타입은 뭔가? int? Node형 배열 아닌가?
        self.no = 0
    ...
    def remove_last(self) -> None:
        '''꼬리 노드를 삭제'''
        if self.head != Null:
            if self.n[self.head].next == Null:  # 노드가 1개(머리 노드)만 있으면
                ...
                while self.n[ptr].next != Null:
                    pre = ptr
                    ptr = self.n[ptr].next
                self.n[pre].next = Null         # pre는 삭제한 뒤의 꼬리 노드가 된다.
    ...
    def remove(self, p: int) -> None:
        ...
        if self.head != Null:
            if p == self.head:
                self.remove_first()     # p가 머리 노드면 머리 노드를 삭제한다.
            else:
                ptr = self.head
