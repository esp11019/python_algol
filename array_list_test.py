...
Menu = Enum('Menu', ['머리노드에삽입','꼬리노드에삽입','머리노드삭제',
                     '꼬리노드삭제','주목노드출력','주목노드이동',
                     '주목노드삭제','모든노드삭제','검색','멤버쉽판단',
                     '모든노드출력','스캔','종료'])
...
lst = ArrayLinkedList(100)      # 연결리스트 생성

while True:
    menu = select_Menu()

    if menu == Menu.머리노드에삽입:
        lst.add_first(int(input('머니 노드에 넣을 값을 입력하세요.: ')))
